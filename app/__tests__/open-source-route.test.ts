...
const mockUserPartnershipFindFirst = prisma.userPartnership.findFirst as jest.Mock;
...
describe("GET /api/open_source", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockUserPartnershipFindFirst.mockResolvedValue(null);
  });
...
describe("DELETE /api/open_source", () => {
  ...
  it("rejects deleting ecosystem conversation cards", async () => {
    mockFindFirst.mockResolvedValue({
      id: 2,
      userId: "user-1",
